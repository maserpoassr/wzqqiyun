...
const state = {
  ...
  winline: [],
  swaped: false,
  gameEnded: false, // Track if game has ended (win/loss/draw)
  gameResult: null, // 'BLACK_WIN' | 'WHITE_WIN' | 'DRAW' | null
}
...
const mutations = {
  ...
  // Reset game state for new game (Requirements: 5.1, 5.2, 5.3)
  ...
  // Set game ended state with result (Requirements: 5.5, 5.6)
  setGameEnded(state, result) {
    state.gameEnded = true
    state.gameResult = result // 'BLACK_WIN' | 'WHITE_WIN' | 'DRAW'
  },
  move(state, pos) {
  ...
  checkWin(state, checkOverline) {
    ...
    for (let dir of dirs) {
      ...
      if (ret) {
        ...
        // Set game ended with winner (Requirements: 5.5)
        // Last move was made by the winner
        const winner = state.position.length % 2 === 1 ? 'BLACK_WIN' : 'WHITE_WIN'
        state.gameEnded = true
    ...
    // Check for draw - board is full with no winner (Requirements: 5.6)
    if (state.position.length >= state.size * state.size) {
      state.gameEnded = true
