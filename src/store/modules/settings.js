...
const state = {
  language: null,
  autoPlayMode: true, // Auto-play mode enabled by default (Requirements: 4.1, 4.2)
  showAnalysisPanel: false, // Hide analysis panel when autoPlayMode is ON (Requirements: 4.3)
  boardStyle: {
    boardColor: '#F4D03F',
    lineColor: '#000000',
    lineWidth: 0.03,
...
const mutations = {
  ...
  toggleAutoPlayMode(state) {
    state.autoPlayMode = !state.autoPlayMode
    // When auto-play is ON, hide analysis panel; when OFF, show it
    state.showAnalysisPanel = !state.autoPlayMode
    saveCookies()
  },
  // Set auto-play mode directly
  setAutoPlayMode(state, enabled) {
    state.autoPlayMode = enabled
    state.showAnalysisPanel = !enabled
    saveCookies()
  },
