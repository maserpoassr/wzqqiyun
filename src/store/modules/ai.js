...
const state = {
  ...
  currentConfig: null,
  hashSize: null,
  autoPlayMode: true, // Auto-play mode enabled by default
  thinkingStartTime: 0, // Track AI thinking duration
  outputs: {
    pos: null,
    swap: null,
    currentPV: 0,
...
const actions = {
  ...
  think({ commit, dispatch, state, rootState, rootGetters }, args) {
    ...
    commit('setThinkingStartTime', Date.now()) // Track thinking start time
    ...
    // Support balanceMode parameter for auto-play
  ...
  // Auto-play specific think action - AI plays the best move
  // Uses standard YXNBEST command for single best move response
  thinkAutoPlay({ dispatch }) {
    // Use standard think without balanceMode for auto-play
    // This sends YXNBEST command which returns a single best move
    // Requirements: 2.5, 3.1
    return dispatch('think', {})
  },
  stop({ commit, state }) {
